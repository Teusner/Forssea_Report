...
\label{chapitre:environnement}
	...
	\section{Environnement marin}
		...
		\subsection{Modélisation des vagues}
			...
			\subsubsection{Dynamique du milieu}
				...
				\begin{eqnarray}
					...
					\eta(\alpha, \beta, z, t) & = & z - \sum_{m=0}^{M} \frac{a_m}{k_m} \cdot \frac{cosh(k(z+H))}{sinh(kh)} \cdot cos(\theta_m)
				\end{eqnarray}
				...
				\begin{eqnarray}
					u(\alpha, \beta, z, t) & = & - \sum_{m=0}^{M} a_m \cdot \omega_m \cdot cos(h_m) \cdot \frac{cosh(k_m(z+H))}{sinh(k_m H)} \cdot cos(\theta_m) \\
					v(\alpha, \beta, z, t) & = & - \sum_{m=0}^{M} a_m \cdot \omega_m \cdot sin(h_m) \cdot \frac{cosh(k_m(z+H))}{sinh(k_m H)} \cdot cos(\theta_m) \\
					w(\alpha, \beta, z, t) & = & \sum_{m=0}^{M} a_m \cdot \omega_m \cdot \frac{sinh(k_m(z+H))}{sinh(k_m H)} \cdot sin(\theta_m)
				\end{eqnarray}
			...
			\subsubsection{Choix du spectre de vagues}
				...
				L'avantage de cette modélisation linéaire des vagues est qu'il est possible d'additionner des vagues de caractéristiques différentes en utilisant le principe de superposition. Ainsi on peut définir un environnement marin réaliste en ajoutant l'effet de plusieurs vagues sur les particules de fluides.

				Pour une optimisation en terme de calculs, il est possible de prendre des nombres d'ondes et donc des pulsations multiples les unes des autres~\cite{tessendorf2001simulating}. Il est aussi possible de choisir des nombres d'ondes tombant sur la grille qui va servir de support pour réaliser le rendu de la surface libre~\cite{tessendorf2001simulating}. Cela permet d'avoir un phénomène périodique qui permet de pré-calculer sur une période tous les paramètres dynamiques de l'environnement, et ainsi de baisser la charge de calculs lors de la simulation. Pour notre cas cela ne sera pas nécessaire, d'abord car la charge de calculs demandée ne ralentit pas la simulation, mais aussi car nous ne voulons pas forcément nous limiter à un phénomène de vagues périodiques.
		...
		\subsection{Forces induites par le milieu}
			...
			\subsubsection{Introduction}
				...
				Les forces qui s'appliquent sur un solide immergé dans un fluide en mouvement sont les forces de Morison et de Froude-Krylov. Elles permettent d'appliquer les forces que subit les objets immergés. Elles possèdent cependant quelques limitations~\cite{sarpkaya2010wave} :

				\begin{itemize}[noitemsep]
					\item[\textbullet] L'accélération du fluide doit être relativement uniforme autour de la structure. Dans notre cas, cela implique que la longueur d'onde des vague doit être grande devant la longueur caractéristique du robot ou de la structure considérée. Comme les robots ont une longueur caractéristique de l'ordre du mètre, on doit respecter $\forall m, \lambda_m \gg 1 m$. \\
			...
			\subsubsection{Force de Froude-Krylov}
				...
				La force de \textit{Froude-Krylov} est la force qui va 

				\begin{equation}
					F_{fk} = \rho \cdot V \cdot \dot{u}
				\end{equation}
			...
			\subsubsection{Force de masse hydrodynamique}
				...
				\begin{equation}
					F_{hm} = \rho \cdot V \cdot C_a \cdot (\dot{u} - \dot{v})
				\end{equation}
