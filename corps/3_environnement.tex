	...
	\section{Environnement marin}
		...
		\subsection{Modèle de houle trochoïdale}
			...
			\subsubsection{Modèle dynamique du MIT}
				...
				Le modèle est basé sur l'étude de vagues dans le plan $(0, \overrightarrow{x}, \overrightarrow{z})$, avec $\overrightarrow{z}$ orienté vers le haut comme représenté sur la \textsc{Figure}~\ref{fig:mit_frame}. On considère l'eau comme un fluide incompressible et que la surface libre est soumise à la pression atmosphérique $P_a$.

				\begin{figure}[!htb]
					\centering
					\begin{tikzpicture}[x={(20:1cm)},y={(90:1cm)},z={(160:0.5cm)},line join=round,]
							...
							\foreach \z in {100, 98, ..., 0}
							{   \pgfmathsetmacro{\drawperc}{or(\z==0,\z==100) ? 1 : 0}
								\ifthenelse{\z=0 \OR \z=100}
								{   \gdef\myc{blue!50!cyan!50!black}}
								{ \gdef\myc{blue!50!cyan!50!white}}
								\filldraw[blue!50!cyan!50!white,xyp=\z/100*3,opacity=1,draw=\myc,draw opacity=1] (0,0) -- (3,0.3) -- (3,1) to[out=170,in=0,looseness=1] (1.5,2) to[out=180,in=20] (0,1.5) -- cycle;
							}
						
							\draw[blue!50!cyan!50!black] (0,1.5,0) -- (0,1.5,3) -- (0,0,3) --(0,0,0);

							\draw[draw=black, fill=gray, opacity=0.2] (0, 0, 0) -- (0, 2.5, 0) -- (3, 2.5, 0) -- (3, 0, 0) -- cycle;
							\draw[->, >=stealth, color=red, thick] (0,0,0) -- (1,0,0) node[right] {$\overrightarrow{x}$};
  							\draw[->, >=stealth, color=NavyBlue, thick] (0,0,0) -- (0,1,0) node[above right] {$\overrightarrow{z}$};
						\end{tikzpicture}
