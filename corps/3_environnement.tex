...
\label{chapitre:environnement}
	...
	\section{Introduction}
		...
		Pour ce faire, nous allons donc d'établir les différents éléments de l'environnement qui vont interagir avec les \gls{ROV}s afin de les intégrer dans le simulateur. Ce qui va principalement influencer le comportement des robots est le milieu marin qui va ajouter la flottabilité, les vagues, des courants marins et du vent. Ensuite, il y a l'ombilical qui relie les \gls{ROV}s au bateau afin de l'alimenter en énergie mais aussi d'avoir un retour d'informations. \color{red}Enfin la présence de structures sous-marines jouent aussi un rôle importante dans les missions des \gls{ROV}s, que ce soit par exemple des piles d'éolienne à inspecter pour le robot \argos{}, son garage, ou bien des structures sous-marines à déplacer pour \atoll{}. Nous allons donc nous intéresser à la manière de simuler ces différents éléments d'environnement pour le simulateur.\color{black}
	...
	\section{Environnement marin}
		...
		\subsection{Forces induites par le milieu}
			...
			\subsubsection{Estimation du volume immergé}
				...
				Le volume immergé et donc le volume de la calotte sphérique est calculable par intégration et le calcul est détaillé en \textsc{Annexe}~\ref{annexe:calotte_spherique} :
