	...
	\section{Environnement marin}
		...
		\subsection{Modélisation des vagues}
			...
			\subsubsection{Introduction}
				...
				Cette modélisation présente l'avantage d'être linéaire. Cela permet d'utiliser le principe de superposition pour additionner plusieurs vagues d'amplitudes et de longueur d'onde différentes afin de constituer un environnement marin réaliste. Il faut cependant s'assurer que les vagues ne déferlent pas afin de ne pas entrer dans le domaine non-linéaire, comme présenté sur la \textsc{Figure}~\ref{fig:hokusai} représentant un tsunami avec des vagues déferlantes. D'après l'échelle de \textit{Beaufort}~\cite{beaufort}, le modèle de vagues linéaire semble respecté jusqu'à \textit{mer 3}. Au delà, des moutons blancs apparaissent à la surface de la mer, et les vagues commencent à déferler, donc le modèle ne sera plus utilisable.

				\begin{figure}
					\centering
					\includegraphics[width=0.8\textwidth]{imgs/tsunami_hokusai.jpg}
		...
		\subsection{Forces induites à un solide}
			
			Les forces de Morison et de Froude-Krylov sont peut coûteuses à calculer et permettent d'appliquer les forces que subit les objets immergés. Elles possèdent cependant quelques limitations~\cite{sarpkaya2010wave} :
