...
def figure_rosace():
    # b = bagreader("scripts/rosace_simulation_2.bag")

    # setpoint = b.message_by_topic('/setpoint')
    # robot_state = b.message_by_topic("/robot_state")

    setpoint = "scripts/rosace_simulation_2/setpoint.csv"
    ...
    odometry = "scripts/rosace_simulation_2/robot_state.csv"
    df_odometry = pd.read_csv(odometry)
    tr = df_odometry['Time'].to_numpy()[::10]
    Xr = df_odometry['p.x'].to_numpy()[np.newaxis, ::10] + x0
    Yr = df_odometry['p.y'].to_numpy()[np.newaxis, ::10] + y0
...
if __name__ == "__main__":
    ...
    if len(args)>1:
        ...
        figname = 'build/imgs/rosace_local_simulation_2.pdf'
